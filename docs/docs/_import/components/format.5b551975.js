export default function format (fmt, len) {
  len = BigInt(len);
  {
    ...
    const m = fmt.match(/^%(?<form>[fega])(?<elen>32|64)$/);
    if (m) {
      ...
      if (m.groups.form === 'f') {
        ...
        return (val, pos) => {
          const sign = (val >> (elen - 1n)) & 1n;
          const fracLen = (elen === 32n) ? 23n : 52n;
          const fracMask = (1n << fracLen) - 1n;
          const expoLen = elen - fracLen - 1n;
          const expoMask = (1n << expoLen) - 1n;
          const expo = (val >> fracLen) & expoMask;
          const expoBiased = expo - (1n << (expoLen - 1n)) + 1n;

          let txtOrig = (
            (sign ? '-' : '') +
            '0x1.' +
            (val & fracMask).toString(16) +
            'p' +
            ((expoBiased < 0n) ? '' : '+') + expoBiased
          );
